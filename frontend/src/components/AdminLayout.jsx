...
import { 
  Activity, Users, FileText, BarChart2, Settings, Search, Bell, ChevronDown, LogOut, ShieldAlert
} from 'lucide-react';
    ...
    <div className="dashboard-layout">
      ...
      <aside className="sidebar">
        ...
        <nav className="sidebar-nav mt-4">
          <a onClick={() => navigate('/admin/dashboard')} className={`nav-item ${location.pathname.includes('/dashboard') ? 'active' : ''}`} style={{cursor: 'pointer'}}>
            <Activity size={20} /><span>System Overview</span>
          </a>
          <a onClick={() => navigate('/admin/employees')} className={`nav-item ${location.pathname.includes('/employees') ? 'active' : ''}`} style={{cursor: 'pointer'}}>
            <Users size={20} /><span>Manage Directory</span>
          </a>
          <a onClick={() => navigate('/admin/leave-reports')} className={`nav-item ${location.pathname.includes('/leave-reports') ? 'active' : ''}`} style={{cursor: 'pointer'}}>
            <FileText size={20} /><span>Leave Reports</span>
          </a>
          <a onClick={() => navigate('/admin/performance-reports')} className={`nav-item ${location.pathname.includes('/performance-reports') ? 'active' : ''}`} style={{cursor: 'pointer'}}>
            <BarChart2 size={20} /><span>Analytics Matrix</span>
          </a>
        </nav>

        <div className="sidebar-footer">
          <a onClick={() => navigate('/admin/settings')} className={`nav-item ${location.pathname.includes('/settings') ? 'active' : ''}`} style={{cursor: 'pointer', marginBottom: '0.5rem'}}>
            ...
            <div className="user-profile" style={{ position: 'relative' }} ref={profileRef}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', width: '100%' }} onClick={() => setShowProfileMenu(!showProfileMenu)}>
                ...
                <div className="user-info">
                  ...
                  <span className="user-role">{user?.designation || user?.role || 'Admin'}</span>
                </div>
