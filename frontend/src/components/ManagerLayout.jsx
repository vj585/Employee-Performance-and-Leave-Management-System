...
import { 
  Activity, Users, Calendar, CheckSquare, Search, Bell, ChevronDown, LogOut, CheckCircle, Star
} from 'lucide-react';
    ...
    <div className="dashboard-layout">
      ...
      <aside className="sidebar">
        ...
        <nav className="sidebar-nav">
          <a onClick={() => navigate('/manager/dashboard')} className={`nav-item ${location.pathname.includes('/dashboard') ? 'active' : ''}`} style={{cursor: 'pointer'}}>
            <Activity size={20} /><span>Overview</span>
          </a>
          <a onClick={() => navigate('/manager/leave-approvals')} className={`nav-item ${location.pathname.includes('/leave-approvals') ? 'active' : ''}`} style={{cursor: 'pointer'}}>
            <CheckSquare size={20} /><span>Leave Approvals</span>
          </a>
          <a onClick={() => navigate('/manager/team-leaves')} className={`nav-item ${location.pathname.includes('/team-leaves') ? 'active' : ''}`} style={{cursor: 'pointer'}}>
            <Calendar size={20} /><span>Team Leaves</span>
          </a>
          <a onClick={() => navigate('/manager/performance-evaluation')} className={`nav-item ${location.pathname.includes('/performance-evaluation') ? 'active' : ''}`} style={{cursor: 'pointer'}}>
            <Star size={20} /><span>Evaluate Team</span>
          </a>
        </nav>
            ...
            <div className="user-profile" style={{ position: 'relative' }} ref={profileRef}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', width: '100%' }} onClick={() => setShowProfileMenu(!showProfileMenu)}>
                ...
                <div className="user-info">
                  ...
                  <span className="user-role">{user?.designation || user?.role || 'Manager'}</span>
                </div>
              ...
              {showProfileMenu && (
                <div style={{ position: 'absolute', top: '120%', right: 0, width: '220px', background: 'var(--surface-base)', border: '1px solid var(--surface-border)', borderRadius: '12px', boxShadow: '0 10px 25px rgba(0,0,0,0.1)', zIndex: 100, overflow: 'hidden' }}>
                  ...
                  <div style={{ padding: '0.5rem' }}>
                    <button onClick={() => { setShowProfileMenu(false); navigate('/manager/profile'); }} style={{ width: '100%', padding: '0.75rem 1rem', textAlign: 'left', background: 'transparent', border: 'none', cursor: 'pointer', color: 'var(--text-primary)', borderRadius: '8px' }}>My Profile</button>
